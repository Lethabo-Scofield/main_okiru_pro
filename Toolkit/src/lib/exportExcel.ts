...
export const exportAuditorExcel = (state: any, options: ExportOptions = {}) => {
  ...
  const summaryRows: any[][] = [
    ...
    ['', 'Voting Rights of Black People', '', 4, fmt(ownCalc.votingRights), '', '', ''],
    ['', 'Economic Interest of Black People', '', 8, fmt(ownCalc.economicInterest), '', '', ''],
    ['', 'Net Value of Black Ownership', '', 8, fmt(ownCalc.netValue), '', '', ''],
    ['', 'Black Women Ownership Bonus', '', 2, fmt(ownCalc.womenBonus), '', '', ''],
    ['', 'New Entrant Bonus', '', 3, fmt(ownCalc.newEntrantBonus), '', '', ''],
    ['Management Control', '', 27, 27, fmt(mgtCalc.total), pct(mgtCalc.total, 27), 'N/A', 'N/A'],
    ['', 'Board Participation (Exec Directors)', '', 3, fmt(mgtCalc.boardExecBlack), '', '', ''],
    ['', 'Board Participation (Non-Exec Directors)', '', 2, fmt(mgtCalc.boardNonExec), '', '', ''],
    ['', 'Board Participation (Black Women)', '', 1, fmt(mgtCalc.boardBWO), '', '', ''],
    ['', 'Other Executive Management (Black)', '', 2, fmt(mgtCalc.otherExecBlack), '', '', ''],
    ['', 'Other Executive Management (BWO)', '', 2, fmt(mgtCalc.otherExecBWO), '', '', ''],
    ['', 'Senior Management', '', 5, fmt(mgtCalc.senior), '', '', ''],
    ['', 'Middle Management', '', 4, fmt(mgtCalc.middle), '', '', ''],
    ['', 'Junior Management', '', 4, fmt(mgtCalc.junior), '', '', ''],
    ['', 'Employees with Disabilities', '', 2, fmt(mgtCalc.disabled), '', '', ''],
    ['', 'Adjusted Recognition for Gender', '', 2, fmt(mgtCalc.adjustedRecognition), '', '', ''],
    ...
    ['', 'Skills Expenditure on Learning Programmes', '', 20, fmt(skillCalc.general), '', '', ''],
    ['', 'Bursaries for Black Students', '', 5, fmt(skillCalc.bursaries), '', '', ''],
    ['Preferential Procurement', '', 25, 25, fmt(procCalc.base), pct(procCalc.base, 25), '≥ 11.6 pts (base)', state.scorecard.procurement.subMinimumMet ? 'Yes' : 'No'],
    ['', 'B-BBEE Procurement from Empowering Suppliers', '', 25, fmt(procCalc.base), '', '', ''],
    ['', 'Designated Group Spend Bonus (51%+ BO)', '', 2, fmt(procCalc.designatedGroup), '', '', ''],
    ['', 'Total Procurement incl. DG Bonus', '', 27, fmt(procCalc.total), '', '', ''],
    ['Enterprise & Supplier Development', '', 15, 15, fmt(esdCalc.total), pct(esdCalc.total, 15), 'N/A', 'N/A'],
    ['', 'Supplier Development (2% of NPAT)', '', 10, fmt(esdCalc.supplierDev), '', '', ''],
    ['', 'Enterprise Development (1% of NPAT)', '', 5, fmt(esdCalc.enterpriseDev), '', '', ''],
    ...
    [''],
    ['TOTAL', '', 127, 127, fmt(state.scorecard.total.score), pct(state.scorecard.total.score, 127), '', state.scorecard.isDiscounted ? 'DISCOUNTED' : 'ALL MET'],
    [''],
    ['RESULT'],
  ...
  const levelRows: any[][] = [
    ...
    ['Enterprise & Supplier Development', 'N/A', fmt(esdCalc.total), '', '', ''],
  ...
  ownershipRows.push(
    [''],
    ['SCORING DETAIL'],
    ['Indicator', 'Target / Threshold', 'Actual', 'Max Points', 'Score Achieved', 'Calculation'],
    ['Voting Rights of Black People', '≥ 25%', `${fmt(ownCalc.rawStats.blackVotingPercentage * 100)}%`, 4, fmt(ownCalc.votingRights), ownCalc.fullOwnershipAwarded ? 'Full ownership: BO ≥ 25% → full 4 pts' : `(${fmt(ownCalc.rawStats.blackVotingPercentage * 100)}% / 25%) × 4`],
    ['Economic Interest of Black People', '25%', `${fmt(ownCalc.rawStats.economicInterestPercentage * 100)}%`, 8, fmt(ownCalc.economicInterest), ownCalc.fullOwnershipAwarded ? 'Full ownership → full 8 pts' : 'Graduated calculation based on years held'],
    ['Net Value of Black Ownership', 'Company Net Value × BO%', '', 8, fmt(ownCalc.netValue), ownCalc.fullOwnershipAwarded ? 'Full ownership → full 8 pts' : '(Net Value attributed to Black shareholders / Company Net Value) × 8'],
    ['Black Women Ownership Bonus', '≥ 10% BWO', `${fmt(ownCalc.rawStats.blackWomenVotingPercentage * 100)}%`, 2, fmt(ownCalc.womenBonus), `BWO ${fmt(ownCalc.rawStats.blackWomenVotingPercentage * 100)}% ${ownCalc.rawStats.blackWomenVotingPercentage >= 0.10 ? '≥' : '<'} 10% target`],
    ['New Entrant Bonus', 'Has New Entrant shareholder', shareholders.some((s: any) => s.blackNewEntrant) ? 'Yes' : 'No', 3, fmt(ownCalc.newEntrantBonus), shareholders.some((s: any) => s.blackNewEntrant) ? 'New entrant present → 3 pts' : 'No new entrant → 0 pts'],
    [''],
    ['TOTAL OWNERSHIP SCORE', '', '', 25, fmt(ownCalc.total), `Capped at 25 pts`],
  ...
  const mgmtRows: any[][] = [
    ['MANAGEMENT CONTROL — ELEMENT 2 (Max 27 Points)'],
    [''],
  ...
  mgmtRows.push(
    [''],
    ['SCORING DETAIL'],
    ['Indicator', 'EAP Target %', 'Actual Black %', 'Actual BWO %', 'Max Points', 'Score Achieved', 'Calculation'],
    ['Board Participation — Executive Directors', '50%', `${fmt(mgtCalc.rawStats.boardBlackVotingPercentage * 100)}%`, '', 3, fmt(mgtCalc.boardExecBlack), `(${fmt(mgtCalc.rawStats.boardBlackVotingPercentage * 100)}% / 50%) × 3`],
    ['Board Participation — Non-Executive Directors', '50%', `${fmt(mgtCalc.rawStats.boardBlackVotingPercentage * 100)}%`, '', 2, fmt(mgtCalc.boardNonExec), `(${fmt(mgtCalc.rawStats.boardBlackVotingPercentage * 100)}% / 50%) × 2`],
    ['Board Participation — Black Women', '25%', '', `${fmt(mgtCalc.rawStats.boardBlackWomenVotingPercentage * 100)}%`, 1, fmt(mgtCalc.boardBWO), `(${fmt(mgtCalc.rawStats.boardBlackWomenVotingPercentage * 100)}% / 25%) × 1`],
    ['Board Subtotal', '', '', '', 6, fmt(mgtCalc.boardTotal), ''],
    ['Other Executive Management — Black', '60%', `${fmt(mgtCalc.rawStats.execBlackVotingPercentage * 100)}%`, '', 2, fmt(mgtCalc.otherExecBlack), `(${fmt(mgtCalc.rawStats.execBlackVotingPercentage * 100)}% / 60%) × 2`],
    ['Other Executive Management — BWO', '30%', '', `${fmt(mgtCalc.rawStats.execBlackWomenVotingPercentage * 100)}%`, 2, fmt(mgtCalc.otherExecBWO), `(${fmt(mgtCalc.rawStats.execBlackWomenVotingPercentage * 100)}% / 30%) × 2`],
    ['Other Exec Subtotal', '', '', '', 4, fmt(mgtCalc.execTotal), ''],
    ['Senior Management', '60%', `${fmt(mgtCalc.rawStats.seniorBlackPercentage * 100)}%`, '', 5, fmt(mgtCalc.senior), `(${fmt(mgtCalc.rawStats.seniorBlackPercentage * 100)}% / 60%) × 5`],
    ['Middle Management', '75%', `${fmt(mgtCalc.rawStats.middleBlackPercentage * 100)}%`, '', 4, fmt(mgtCalc.middle), `(${fmt(mgtCalc.rawStats.middleBlackPercentage * 100)}% / 75%) × 4`],
    ['Junior Management', '88%', `${fmt(mgtCalc.rawStats.juniorBlackPercentage * 100)}%`, '', 4, fmt(mgtCalc.junior), `(${fmt(mgtCalc.rawStats.juniorBlackPercentage * 100)}% / 88%) × 4`],
    ['Employees with Disabilities', '3% of total', `${fmt(mgtCalc.rawStats.disabledBlackPercentage * 100)}%`, '', 2, fmt(mgtCalc.disabled), `(${fmt(mgtCalc.rawStats.disabledBlackPercentage * 100)}% / 3%) × 2`],
    ['Adjusted Recognition for Gender', 'Has female data', employees.some((e: any) => e.gender === 'Female') ? 'Yes' : 'No', '', 2, fmt(mgtCalc.adjustedRecognition), employees.some((e: any) => e.gender === 'Female') ? 'Female representation present → 2 pts' : 'No female data → 0 pts'],
    [''],
    ['TOTAL MANAGEMENT CONTROL', '', '', '', 27, fmt(mgtCalc.total), 'Capped at 27 pts'],
  );
  ...
  const skillsRows: any[][] = [
    ...
    ['General Skills Target (3.5%)', leviable * 0.035, '3.5% × Leviable Amount'],
    ['Bursary Target (2.5%)', leviable * 0.025, '2.5% × Leviable Amount'],
    ['Black People Target (6%)', leviable * 0.06, '6% × Leviable Amount (EAP indicator)'],
    ['Black Women Target (3%)', leviable * 0.03, '3% × Leviable Amount (EAP indicator)'],
    ['Black Disabled Target (0.3%)', leviable * 0.003, '0.3% × Leviable Amount (EAP indicator)'],
    [''],
    ['ACTUAL SPEND'],
    ['Total Black Skills Spend', skillCalc.actualSpend, pct(skillCalc.actualSpend, leviable) + ' of Leviable Amount'],
    ['Bursary Spend (Black students)', skillCalc.actualBursarySpend, pct(skillCalc.actualBursarySpend, leviable) + ' of Leviable Amount'],
    ['Black Women Spend', skillCalc.rawStats.blackWomenSpend, ''],
    ...
    ['Skills Expenditure on Learning Programmes', leviable * 0.035, skillCalc.actualSpend, pct(skillCalc.actualSpend, leviable * 0.035), 20, fmt(skillCalc.general), `(${pct(skillCalc.actualSpend, leviable * 0.035)} / 100%) × 20`],
    ['Bursaries for Black Students', leviable * 0.025, skillCalc.actualBursarySpend, pct(skillCalc.actualBursarySpend, leviable * 0.025), 5, fmt(skillCalc.bursaries), `(${pct(skillCalc.actualBursarySpend, leviable * 0.025)} / 100%) × 5`],
    [''],
    ['TOTAL SKILLS SCORE', '', '', '', 25, fmt(skillCalc.total), 'Capped at 25 pts'],
    ['Sub-minimum Threshold', '', '', '', '', '≥ 10 pts', '40% of 25 max points'],
    ['Sub-minimum Met', '', '', '', '', skillCalc.subMinimumMet ? 'Yes' : 'No', `Score ${fmt(skillCalc.total)} ${skillCalc.total >= 10 ? '≥' : '<'} 10 pts`],
    [''],
    ['EAP INDICATORS (Information — not scored in Generic Scorecard)'],
    ['Indicator', 'Target %', 'Target (ZAR)', 'Actual Spend (ZAR)', 'Achievement', 'Max Indicator Pts', 'Indicator Score'],
    ['Black People', '6%', leviable * 0.06, skillCalc.eapIndicators.blackPeople.spend, pct(skillCalc.eapIndicators.blackPeople.spend, leviable * 0.06), 3, fmt(skillCalc.eapIndicators.blackPeople.score)],
    ['Black Women', '3%', leviable * 0.03, skillCalc.eapIndicators.blackWomen.spend, pct(skillCalc.eapIndicators.blackWomen.spend, leviable * 0.03), 3, fmt(skillCalc.eapIndicators.blackWomen.score)],
    ['Black Disabled', '0.3%', leviable * 0.003, skillCalc.eapIndicators.disabled.spend, pct(skillCalc.eapIndicators.disabled.spend, leviable * 0.003), 4, fmt(skillCalc.eapIndicators.disabled.score)],
    ['Absorption Rate', '2.5%', '', `${skillCalc.eapIndicators.absorption.count} of ${skillCalc.eapIndicators.absorption.total} learners`, `${fmt(skillCalc.eapIndicators.absorption.rate * 100)}%`, '', ''],
    [''],
    ['SPEND BY CATEGORY'],
  ...
  const procRows: any[][] = [
    ['PREFERENTIAL PROCUREMENT — ELEMENT 4 (Max 25 Base + 2 DG Bonus = 27 Points)'],
    [''],
  ...
  procRows.push(
    ...
    ['B-BBEE Procurement from Empowering Suppliers', `80% of TMPS (R ${((state.procurement?.tmps || 0) * 0.8).toLocaleString('en-ZA')})`, `R ${totRecognised.toLocaleString('en-ZA')} recognised`, 25, fmt(procCalc.base), `(Recognised / Target) × 25 = (${pct(totRecognised, (state.procurement?.tmps || 0) * 0.8)}) × 25`],
    ['Designated Group Bonus (51%+ BO)', '12% of TMPS', `R ${totDG.toLocaleString('en-ZA')}`, 2, fmt(procCalc.designatedGroup), `(DG Spend / TMPS) × 10, capped at 2`],
    [''],
    ['TOTAL PROCUREMENT (Base)', '', '', 25, fmt(procCalc.base), 'Used for sub-minimum test'],
    ['TOTAL PROCUREMENT (incl. DG Bonus)', '', '', 27, fmt(procCalc.total), 'Final procurement score'],
    ['Sub-minimum Threshold', '', '', '', '≥ 11.6 pts', 'Applied to base score only (excl. DG bonus)'],
  ...
  esdRows.push(
    ...
    ['Supplier Development (2% of NPAT)', Math.abs(state.client.npat) * 0.02, sdRecognised, pct(sdRecognised, Math.abs(state.client.npat) * 0.02), 10, fmt(esdCalc.supplierDev), `(Recognised / Target) × 10`],
    ...
    [''],
    ['TOTAL ESD SCORE', '', '', '', 15, fmt(esdCalc.total), 'Capped at 15 pts'],
  );
  ...
  const auditRows: any[][] = [
    ...
    [''],
    ['SCORECARD RESULT'],
    ['Total Points', `${fmt(state.scorecard.total.score)} / 127`, ''],
    ...
    ['Management Control', fmt(mgtCalc.total), 27, 'N/A'],
    ['Skills Development', fmt(skillCalc.total), 25, skillCalc.subMinimumMet ? 'Met' : 'FAILED'],
    ['Procurement (base)', fmt(procCalc.base), 25, procCalc.subMinimumMet ? 'Met' : 'FAILED'],
    ['Procurement (incl. DG)', fmt(procCalc.total), 27, ''],
    ['ESD', fmt(esdCalc.total), 15, 'N/A'],
    ['SED', fmt(sedCalc.total), 5, 'N/A'],
    ['YES Initiative', fmt(state.scorecard.yesInitiative?.score || 0), 5, 'N/A'],
    ['TOTAL', fmt(state.scorecard.total.score), 127, ''],
    [''],
    ['FLAGS & OBSERVATIONS'],
  ];
  ...
  const pillars = [
    ...
    { name: 'Management Control', score: mgtCalc.total, target: 27 },
    { name: 'Skills Development', score: skillCalc.total, target: 25 },
    { name: 'Procurement (base)', score: procCalc.base, target: 25 },
    { name: 'Enterprise & Supplier Dev', score: esdCalc.total, target: 15 },
